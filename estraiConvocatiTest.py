...
if create:
    ...
    dataframe_arricchito_0 = pd.concat([
        duckdb.query(f"""
            select *
            from df
            join season s
    ...
    dataframe_arricchito = duckdb.query("""
        select
              *
            , dense_rank() over (partition by cod order by data desc) as GIORNATA_CALCOLATA
        from dataframe_arricchito_0
        """).df()

    [consigli_di_giornata_test(ultima_giornata,
                               n_giornate,
                               stagione,
                               dataframe_arricchito,
